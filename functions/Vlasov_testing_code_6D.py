...
def iterateEB(params, grids, mass, q, advanced_params = False):
    ...
    for i in range(len(mass)):
        ...
        # Define advanced parameters if applicable
        if advanced_params:
            J1, J2, J3  = EB_calc.J_compute(result_arrays, q[i], grids)
            # Add up current contributions for each fluid.
            J1tot   += J1
            J2tot   += J2
            J3tot   += J3
    ...
    if advanced_params:
        E               = np.array([E1tot, E2tot, E3tot])
        B               = np.array([B1tot, B2tot, B3tot])
        J               = np.array([J1tot, J2tot, J3tot])
        sigma_P         = EB_calc.sigma_compute(E, B, J)
        params['fJ1']   = scipy.interpolate.RegularGridInterpolator(uniqs, J1tot, method='linear')
        params['fJ2']   = scipy.interpolate.RegularGridInterpolator(uniqs, J2tot, method='linear')
        params['fJ3']   = scipy.interpolate.RegularGridInterpolator(uniqs, J3tot, method='linear')
        params['fsP']   = scipy.interpolate.RegularGridInterpolator(uniqs, sigma_P, method='linear')
...
def iterateEB_until_result(params, grids, fluids, advanced_params = False, rmserrormax = 1e-6):
    ...
    while error     > rmserrormax:
        ...
        force, stability, result_arrays, params = iterateEB(params, grids, fluids[0], fluids[1], advanced_params)
